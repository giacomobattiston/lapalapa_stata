...
*Lucia User
global main "C:\Users\CornoL\Dropbox\Lapa-Lapa\LuciaElianaGiacomo\logistics\03_data/04_final_data"


*global main "/Users/giacomobattiston/Dropbox/Lapa-Lapa/LuciaElianaGiacomo/logistics/03_data/04_final_data"
...
global bc_var = "wealth_index size mig_plan mig_prep mig_des outside_contact_no age gender mig_classmates expectation_wage  employed asylum studies beaten duration journey_cost die_boat die_bef_boat moth_school fath_school"


	
//the global must be changed depending on the strategy of stratification because of convergence issues
global cov_var = "mig_plan mig_prep mig_des mig_classmates"
...
randomize, groups(4) block(`strata') seed(112358) replace balance($cov_var) jointp(0.2)  minruns(1000) 
...
orth_out $bc_var using "$main/t_test_dummy_tot.xls", by(_assignment) se test pcompare stars replace covariates(strata_n*) count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")
orth_out $bc_var using "$main/t_test_nodummy_tot.xls", by(_assignment) se test pcompare stars replace count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")
...
esttab matrix(normalized) using "$main/normalized_tot.csv", replace
...
orth_out $bc_var using "$main/t_test_nodummy_1.xls", by(_assignment) se test pcompare stars replace count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")
...
esttab matrix(normalized) using "$main/normalized_1.csv", replace
...
orth_out $bc_var using "$main/t_test_nodummy_2.xls", by(_assignment) se test pcompare stars replace count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")
...
esttab matrix(normalized) using "$main/normalized_2.csv", replace
