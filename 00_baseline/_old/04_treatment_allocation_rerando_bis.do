...
*global bc_var = " age gender pular_language sousou_language malinke_language outside_contact_no italy_duration italy_beaten italy_die_boat log_wage employed "
		
// SELECTING THE VARIABLE FOR THE BALANCE 
global bc_var = " age gender pular_language sousou_language malinke_language mig_desire outside_contact_no mig_classmates log_wage employed asylum studies italy_duration"


** Collapse at cluster level
collapse (mean) $bc_var  wealth_index italy_die_boat  italy_beaten beaten duration journey_cost die_boat die_bef_boat italy_die_bef_boat size italy_journey_cost spain_journey_cost planning prepare spain_beaten spain_duration spain_die_boat spain_die_bef_boat expectation_wage fav_mig it_favorite_road sp_favorite_road, by(schoolid)



// STRATA : SCHOOL SIZE / DHS INDEX
xtile quart_size=size, nq(4)
*xtile quart_wealth = wealth_index, nq(2)

local strata "quart_size"
...
randomize, groups(4) block(`strata') balance($bc_var) jointp(0.5) seed(1) minruns(1000) replace
