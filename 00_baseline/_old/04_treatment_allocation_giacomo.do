...
global bc_var = "wealth_index size mig_plan mig_prep mig_des outside_contact_no age gender pular_language sousou_language malinke_language mig_classmates log_wage expectation_wage  employed asylum studies beaten duration journey_cost die_boat die_bef_boat moth_school fath_school"
	
//the global must be changed depending on the strategy of stratification because of convergence issues
global cov_var = "  mig_plan age gender beaten employed  "
...
local strata "xtile_size xtile_wealth"
...
***¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
***  5 TREATMENT ALLOCATION     WITHOUT COVARIATES OPTION                   
***_____________________________________________________________________________

randomize, groups(4) block(`strata') seed(1234) replace
*balance($cov_var) jointp(0.2) seed(12345) minruns(100) 
...
orth_out $bc_var using "$main/bc_var_giacomo.xls", by(_assignment) se test pcompare stars replace covariates(strata_n*) count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")

pause 

twoway (scatter latitude longitude if _assignment == 1, mc(black)) (scatter latitude longitude if _assignment == 2, mc(blue)) (scatter latitude longitude if _assignment == 3, mc(red)) (scatter latitude longitude if _assignment == 4, mc(yellow))



***¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
***  7 TREATMENT ALLOCATION     WITH COVARIATES OPTION                   
***_____________________________________________________________________________


randomize, groups(4) block(`strata') seed(1234) replace balance($cov_var) jointp(0.2)  minruns(1000) 
...
orth_out $bc_var using "$main/bc_var_giacomo_cov.xls", by(_assignment) se test pcompare stars replace covariates(strata_n*) count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")
