...
randomize, groups(4) seed(123) replace
...
orth_out $bc_var using "$main/nostrat1.xls", by(_assignment) se test pcompare stars replace count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")
...
randomize, groups(4) seed(123) replace balance($cov_var) jointp(0.2)  minruns(1000) 
...
orth_out $bc_var using "$main/nostrat2.xls", by(_assignment) se test pcompare stars replace count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")
