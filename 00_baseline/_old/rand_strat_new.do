...
global bc_var = "wealth_index size mig_plan mig_prep mig_des outside_contact_no age gender pular_language sousou_language malinke_language mig_classmates  expectation_wage  employed asylum studies beaten duration journey_cost die_boat die_bef_boat moth_school fath_school"
...
randomize, groups(4) block(`strata') seed(321) replace
...
*orth_out $bc_var using "$main/strat1_new.xls", by(_assignment) se test pcompare stars replace covariates(strata_n*) count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")

mata: table_tot = J(46, 6, .)
local l 0
...
foreach var of global bc_var {
	...
	mata: table_tot[`l'*2 - 1, 1] = (`m2' - `m1')/(sqrt(`v1'+`v2'))
	mata: table_tot[`l'*2 - 1, 2] = (`m3' - `m1')/(sqrt(`v1'+`v3'))
	mata: table_tot[`l'*2 - 1, 3] = (`m4' - `m1')/(sqrt(`v1'+`v4'))
	mata: table_tot[`l'*2 - 1, 4] = (`m3' - `m2')/(sqrt(`v2'+`v3'))
	mata: table_tot[`l'*2 - 1, 5] = (`m4' - `m2')/(sqrt(`v2'+`v4'))
	mata: table_tot[`l'*2 - 1, 6] = (`m4' - `m3')/(sqrt(`v3'+`v4'))

}

mata: st_matrix("normalized", table_tot)

matrix colnames normalized = risk_control risk_controlecon_control double_control risk_econ risk_double econ_double	
matrix rownames normalized = wealth_index . size . mig_plan . mig_prep . mig_des . outside_contact_no . age . gender . pular_language . sousou_language . malinke_language . mig_classmates . expectation_wage  . employed . asylum  . studies . beaten . duration . journey_cost . die_boat . die_bef_boat . moth_school . fath_school. 
