...
merge 1:m lycee_name_string using "$main/output/baseline/questionnaire_baseline_clean_rigourous_cleaning.dta"
keep if _merge==3
drop _merge
...
orth_out $bc_var using "$main/output/randomization/t_test_dummy_tot.xls", by(_assignment) se test pcompare stars replace covariates(strata_n*) count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")
orth_out $bc_var using "$main/output/randomization/t_test_nodummy_tot.xls", by(_assignment) se test pcompare stars replace count vcount title(Balance checks)  armlabel("Control group" "Treatment risk" "Treatment condition in UE" "Double Treatment")
...
esttab matrix(normalized) using "$main/output/randomization/normalized_tot.csv", replace
...
*keep latitude longitude _assignment
*export delimited using "gmaps_0103209", replace
