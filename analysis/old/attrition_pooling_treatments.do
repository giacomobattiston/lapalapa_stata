...
gen treated = treatment_status > 1
...
m: obs[1,2] = strtoreal(st_local("NN"))
m: obs[1,3] = strtoreal(st_local("NT"))
m: obs[1,1] = obs[1,2] + obs[1,3] 

*sensibilization

qui count if treated == 0 & sensibilized == 1
local NN = `r(N)'

qui count if treated == 1 & sensibilized == 1
local NT = `r(N)'


m: obs[2,2] = 0
m: obs[2,3] = strtoreal(st_local("NT"))
m: obs[2,1] = obs[2,2] + obs[2,3]
...
qui count if treated == 0 & duplic == 1
local NN = `r(N)'

qui count if treated == 1 & duplic == 1
local NT = `r(N)'

qui reg duplic treatment_status if (treated == 0 | treated == 1) 
local t = _b[treatment_status]/_se[treatment_status]
local pt =2*ttail(e(df_r),abs(`t'))



m: obs[4,2] = strtoreal(st_local("NN"))
m: obs[4,3] = strtoreal(st_local("NT"))
m: obs[4,1] = obs[4,2] + obs[4,3] 
...
m: st_matrix("obs", obs)
matrix colnames obs = `" "All"" Control" "Treated" "'
matrix rownames obs = `" "Baseline N"   "Sensibilization N" "\% of Base N"  "Endline N" "\% of Base N" "'

outtable  using observations_pooled, mat(obs) nobox replace caption("Observations by treatment arm and stage \label{obs}")
...
reg duplic treated, cluster(schoolid)
est sto reg1

reg duplic treated $outcomes, cluster(schoolid)
est sto reg2

reg duplic treated $outcomes $controls, cluster(schoolid)
est sto reg3


esttab reg1 reg2 reg3 using ///
	"attrition_outcomes_controls_clustered_pooled.tex", replace ///
	coeflabels(treated "Treated"  2.strata "Big school") ///
	nonumbers title("Participation in midline, treatment, outcomes, and controls \label{attritioncluster}") /// 
	mtitles("(1)" "(2)"  "(3)"  ) nobaselevels ///
