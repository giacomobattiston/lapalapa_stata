...
global main "giacomobattiston\Dropbox\Lapa-Lapa\LuciaElianaGiacomo"
...
global controls = `" "i.treatment_status" "' ///
				+ `" "i.treatment_status i.strata" "' ///
				+ `" "i.treatment_status 	y" "' ///
				+ `" "i.treatment_status i.strata y" "'
	
global controls_names = `" "with no controls" "' ///
				+ `" "controlling for stratitification dummy" "' ///
...
foreach control_vl in $controls {
	...
	"horizontal_migration_`j'.tex", replace ///
	...
	y "Outcome at baseline") ///
	nonumbers title(Migration intentions `w_c' \label{intentionstable`j'}) /// 
...
coefplot  $migration_plot_list, drop(_cons y 2.strata ) xline(0) ///
...
foreach route_u in $routes_list {
	...
	foreach control_vl in $controls {
		...
		esttab $regression_list using ///
			"horizontal_risks_`route_u'_`j'.tex", replace ///
			coeflabels(1.treatment_status "Control" 2.treatment_status ///
			"Risk Treatment" 3.treatment_status "Economic Treatment" ///
			4.treatment_status "Double Treatment" 2.strata "Big school" ///
			y "Outcome at baseline") ///
			nonumbers title(Risk perceptions for route through `route_u' `w_c' \label{risks`route_u'table`j'}) /// 
			mtitles("(1)" "(2)"  "(3)"  "(4)" "(5)" "(6)" "(7)" "(8)") ///
			nobaselevels ///
			addnotes($risks_table_legend)
	}

	coefplot  $risks_plot_list, drop(_cons y 2.strata ) xline(0) ///
		graphregion(color(white))  ///
		coeflabels(2.treatment_status = "Risk Treatment" ///
		3.treatment_status = "Economic Treatment" ///
		4.treatment_status = "Double Treatment") 

	graph export risks_`route_u'.png, replace
		
	coefplot  $risks_plot_nolog_list, drop(_cons y 2.strata ) xline(0) ///
		graphregion(color(white))  ///
		coeflabels(2.treatment_status = "Risk Treatment" ///
		3.treatment_status = "Economic Treatment" ///
		4.treatment_status = "Double Treatment") 

	graph export risks_`route_u'_nolog.png, replace		
...
foreach control_vl in $controls {
	...
	esttab $regression_list using ///
		"horizontal_economic_`j'.tex", replace ///
		coeflabels(1.treatment_status "Control" 2.treatment_status ///
		"Risk Treatment" 3.treatment_status "Economic Treatment" ///
		4.treatment_status "Double Treatment" 2.strata "Big school" ///
		y "Outcome at baseline") ///
		nonumbers title(Perceptions about economic outcomes `w_c' \label{economictable`j'}) /// 
...
coefplot  $economic_plot_list, drop(_cons y 2.strata) xline(0) ///
	graphregion(color(white))  ///
	coeflabels(2.treatment_status = "Risk Treatment" ///
	3.treatment_status = "Economic Treatment" ///
	4.treatment_status = "Double Treatment")
...
coefplot  $economic_plot_list_nolog, drop(_cons y 2.strata ) xline(0) ///
	graphregion(color(white))  ///
	coeflabels(2.treatment_status = "Risk Treatment" ///
	3.treatment_status = "Economic Treatment" ///
	4.treatment_status = "Double Treatment")

graph export economic_nolog.png, replace
